...
> = Plugin<ReactNode, TSlots, TContext>;
...
export type ReactRegistrySlotComponent<
  TSlots extends SlotMap,
  TContext extends PluginContext = PluginContext,
> = <K extends keyof TSlots>(props: ReactSlotProps<TSlots, K, TContext>) => ReactNode;
...
export function createReactSlotRegistry<
  TSlots extends SlotMap,
  TContext extends PluginContext = PluginContext,
>(
  ...
  return createSlotRegistry<ReactNode, TSlots, TContext>(
    renderer,
    "react:slot-registry",
    context,
    options,
...
function getSlotProps<
  ...
  K extends keyof TSlots,
  TContext extends PluginContext = PluginContext,
>(props: ReactSlotProps<TSlots, K, TContext>): TSlots[K] {
  const {
    children: _children,
    mode: _mode,
    name: _name,
...
>(props: ReactSlotProps<TSlots, K, TContext>): ReactNode {
  ...
  const pendingRenderReportsRef = useRef<
    Map<string, { pluginId: string; slot: string; error: Error }>
  >(new Map());
  ...
  if (props.mode === "replace") {
    if (entries.length === 1) {
      ...
      const rendered = renderEntry(singleEntry, props.children);
      if (rendered === null || rendered === undefined || rendered === false) {
        return <>{props.children}</>;
      }
