...
const DEFAULT_GLOBAL_ROASTER_CONFIG = {
  skills: {
    roots: [],
...
function resolveGlobalRoasterRootDir(env = process.env) {
  ...
  if (fromRoaster.trim().length > 0) {
    return resolve(resolveMaybeAbsolute(process.cwd(), fromRoaster), "..");
  }
...
function seedGlobalConfig(globalRoot) {
  ...
  const configPath = join(globalRoot, "roaster.json");

  if (!existsSync(configPath)) {
    writeFileSync(configPath, `${JSON.stringify(DEFAULT_GLOBAL_ROASTER_CONFIG, null, 2)}\n`, "utf8");
    console.log(`pi-roaster: created global config at ${configPath}`);
    return;
  }
  ...
  try {
    ...
    if (!isRecord(existing)) {
      console.warn(`pi-roaster: skipped renewing config (not an object): ${configPath}`);
      return;
    }
    const merged = deepMerge(DEFAULT_GLOBAL_ROASTER_CONFIG, existing);
    writeFileSync(configPath, `${JSON.stringify(merged, null, 2)}\n`, "utf8");
    console.log(`pi-roaster: renewed global config at ${configPath}`);
  } catch {
    console.warn(`pi-roaster: skipped renewing config (invalid JSON): ${configPath}`);
  }
}
...
function seedGlobalSkills(globalRoot, runtimeBinaryPath) {
  ...
  if (!existsSync(sourceSkillsDir)) {
    console.warn(`pi-roaster: bundled skills not found at ${sourceSkillsDir}`);
    return;
  }
  const targetSkillsDir = join(globalRoot, "skills");
  copyDirectoryContents(sourceSkillsDir, targetSkillsDir);
  console.log(`pi-roaster: renewed global skills at ${targetSkillsDir}`);
}

function main() {
  const { platform, arch } = process;
  const libcFamily = getLibcFamily();
  const globalRoot = resolveGlobalRoasterRootDir(process.env);

  try {
    seedGlobalConfig(globalRoot);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`pi-roaster: failed to seed global config: ${message}`);
  }
