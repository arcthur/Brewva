...
describe("hosted auth store", () => {
  test("reads opencode-style oauth access tokens for openai-codex", async () => {
    ...
    try {
      ...
      expect(await authStore.getApiKey("openai-codex")).toBe("legacy-access-token");
    } finally {
      restoreNow();
    }
  });

  test("refreshes expired opencode-style oauth tokens for openai-codex", async () => {
    const now = 2_000_000;
    ...
    try {
      ...
      expect(await authStore.getApiKey("openai-codex")).toBe("fresh-access-token");
      expect(await authStore.getApiKey("openai-codex")).toBe("fresh-access-token");
      expect(fetchCalls).toBe(1);
      expect(authStore.get("openai-codex")).toEqual({
        type: "oauth",
